...
export default function StudentProfile() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      const { data: roleData } = await supabase.from("users").select("role").eq("id", authUser.id).maybeSingle();
    ...
    try {
      ...
      const { data: updated, error } = await supabase
        ...
        .update({
          ...
          // Completing the parcours section here counts as completing the
          // signup quiz — it asks the same questions. `undefined` is dropped
